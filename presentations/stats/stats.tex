...
\begin{frame}{Tie Generating Process: Latent Edges}
	...
	\footnotesize
		...
		\end{equation*}
		\normalsize
		where $\delta$ is a real-valued intercept controlling recipient size parameter and  \footnotesize$Z(\delta,\mbox{log}(\lambda_i^{(d)})) = (\prod\limits_{j \in \mathcal{A}_{\backslash i}}(\mbox{exp}\{\delta+\mbox{log}(\lambda_{ij}^{(d)})\}+1)) - 1 $\hspace{0.1cm}\normalsize is the normalizing constant\\ \vspace{0.1cm}
 \begin{figure}
...
\begin{frame}{Joint Distribution Tests: Getting it Right (Geweke, 2004)}
	\bni
	\item \textcolor{blue}{Forward sampling}: draws parameters from the prior and then generate data conditional on the parameters\vspace{0.3cm}
	\item  \textcolor{blue}{Backward sampling}: estimate the parameters from inference algorithm and then generate data conditional on the posterior estimates\vspace{0.2cm}
	 \begin{figure}
	 	\includegraphics[width=0.76\textwidth]{figures/Schein.jpg}
	 \end{figure}	
	\ei
\end{frame}
...
\begin{frame}{Conclusion}
 \bni
 \item Interaction Pattern (IP): cluster of topics that share network properties  	\vspace{0.4cm}
