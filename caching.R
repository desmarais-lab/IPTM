...
#use gamma-exponential conjugacy
...
plot(VanceMCMC$E[,10], type="l", main="Empirical Entropy of IP", xlab="Iterations", ylab="Entropy")
...
exp(sapply(1:3, function(c){rowMeans(Bnew[[c]])}))
...
W2<-Wtable[[2]]/sum(Wtable[[2]])
W2[order(W2, decreasing=TRUE)][1:10]
W13<-Wtable[[13]]/sum(Wtable[[13]])
W13[order(W13, decreasing=TRUE)][1:10]
W8<-Wtable[[8]]/sum(Wtable[[8]])
W8[order(W8, decreasing=TRUE)][1:10]
W15<-Wtable[[15]]/sum(Wtable[[15]])
W15[order(W15, decreasing=TRUE)][1:10]
W7<-Wtable[[7]]/sum(Wtable[[7]])
W7[order(W7, decreasing=TRUE)][1:10]
W3<-Wtable[[3]]/sum(Wtable[[3]])
W3[order(W3, decreasing=TRUE)][1:10]
W6<-Wtable[[6]]/sum(Wtable[[6]])
W6[order(W6, decreasing=TRUE)][1:10]
W16<-Wtable[[16]]/sum(Wtable[[16]])
...
IP2<-table(names(allwords))/sum(table(names(allwords)))
IP2[order(IP2, decreasing=TRUE)][1:10]

allwords<-unlist(Zsummary3)
IP3<-table(names(allwords))/sum(table(names(allwords)))
IP3[order(IP3, decreasing=TRUE)][1:10]
