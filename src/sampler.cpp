...
double logPLC(List finalZ, List zcnew, IntegerVector currentC, List tableW, List corpusW, List textlist, double alpha, NumericVector mvec, double delta, NumericVector nvec){
  ...
  for (int d=0; d<finalZ.size(); d++){
    ...
    for (int n=0; n<finalZd.size(); n++){
      ...
      if (zcnewd[finalZd[n]-1] > 0) {ifelse1=1;} else {ifelse1=0;}
      if (tableWdn[textlistd[n]-1] > 0) {ifelse2=1;} else {ifelse2=0;}
      out2[n]=log(zcnewd[finalZd[n]-1]-ifelse1+alpha*mvec[finalZd[n]-1])-log(finalZd.size()-1+alpha)+log(tableWdn[textlistd[n]-1]-ifelse2+delta*nvec[n])-log(corpusWdn.size()-ucorpusWdn.size()+delta);
    }
    out[d]=sum(out2);
  }
  return sum(out);
...
NumericMatrix allxmatC(NumericMatrix edge, List pre, int i, IntegerVector A){
  ...
  for (int b=0; b<A.size(); b++){		
    if (A[b]!=i){A1[iter]=A[b];}
    if (A[b]!=i){iter=iter+1;}
  }
  for (int a=0; a<A1.size(); a++){
    ...
    for (int ii=0; ii<ijlist.size(); ii++){send=send+exp(-0.05*ijlist[ii]);}
    for (int jj=0; jj<jilist.size(); jj++){receive=receive+exp(-0.05*jilist[jj]);}
    for (int h=0; h<A2.size(); h++){
      ...
      for (int i=0; i<ialist.size(); i++){triangle1+=exp(-0.05*ialist[i]);}
      for (int j=0; j<ajlist.size(); j++){triangle2+=exp(-0.05*ajlist[j]);}
      for (int i=0; i<ailist.size(); i++){triangle3+=exp(-0.05*ailist[i]);}
      for (int j=0; j<jalist.size(); j++){triangle4+=exp(-0.05*jalist[j]);}
...
// // [[Rcpp::export]]
// NumericVector Skupdate(NumericVector vec, List cktable){
//   // Skupdate: numerator part used for parameter optimization
//   NumericVector s(vec.size());
//   for (int k=0; k<vec.size(); k++){
//     double d=0;
//     IntegerVector nowtable=cktable[k];
