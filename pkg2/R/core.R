...
#making all indicators
Netstats2 = function(historyIP, node, sender, netstat) {
  # Calculate the chosen network statistics given the history of interactions
  #
  # Args 
  #  historyIP list containing the weighted history of interactions for each IP
...
DataAug2 = function(iJi_d, lambda_d, delta, timeinc_d, observedi) {
  ...
  y = delta * timeinc_d * lambdadiff - log(delta * lambda_d)
  denom = log(1 + exp(y))
  denom[y > 35] = y[y > 35]
...
MCMC2 = function(edge, node, textlist, vocabulary, nIP, K, delta.B, 
  ...
  # initialize alpha, mvec, delta, nvec, eta, lvec, and gammas
    ...
    sigma = 1
    L = 4
    P = 1 + (2 * L) * ("dyadic" %in% netstat) + (4 * L^2 ) * ("triadic" %in% netstat) + (2 * L) *("degree" %in% netstat)
    bmat = list()
	for (IP in 1L:nIP) {
		bmat[[IP]] = matrix(0, nrow = P, ncol = (n3 - burn) / thin)
  	}
  ...
  for (o in 1L:outer) {
    ...
    for (d in seq(along = edge)) {
   	 	history.t = History(edge, p.d, node, as.numeric(edge[[d]][3]))
   	 	X = lapply(node, function(i) {
  	            Netstats2(history.t, node, i, netstat)
            })
   	 	XB = lapply(1L:nIP, function(IP) {
    	  	 t(vapply(node, function(i) {
    	     MultiplyXB(X[[i]][[IP]], Beta.old[[IP]])
