...
\end{itemize} 
 ...
 \begin{algorithm}[H]
 	...
 	\For{i=1 to A}{
 	\For{j=1 to A}{
 		\If{i $\neq$ j}{ set $\lambda^{(c)}_{ij}(t)=\lambda_0\cdot \mbox{exp}(\boldsymbol{\beta}^{(c)T}\boldsymbol{x}(i, j, t))\cdot 1\{j \in \mathcal{J}^{(c)}_{(i, t)}\}$ }
 ...
 \begin{figure}[ht]
 	...
 	\node[main] (delta) [above=of phi,label=center:$\delta$] { };
 	 	\node[main] (beta) [below=of theta,label=center:$\beta^{(c)}$] { };
 	 		\node[main] (sigma) [left=of beta,label=center:$\sigma^2$] { };
 	 		 	 			\node[main, fill = black!10] (N) [right=of beta,label=center:$N_{ij}^{(c)}$] { };
 	...
 	\end{tikzpicture}
 	\caption{Plate notation of IPME}
 	\label{fig:plate}
 \end{figure}
 \normalsize
...
1. $\mbox{send}(i, j, t)=\sum\limits_{d: t_d<t} I\{i\rightarrow j\}\cdot g(t-t_d)$\\
