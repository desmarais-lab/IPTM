...
\end{equation}
 ...
 \end{aligned}
  ...
  Using Bayes' theorem (See APPENDIX B for conditional probabilty of the last term), we have
   \begin{equation}
   \begin{aligned} &=\Big[\gamma_{c}\Big]\Big[\mbox{exp}\Big\{{-\big(\sum\limits_{i \in \mathcal{A}^{(c)}}\sum\limits_{j\in \mathcal{A}^{(c)}}\lambda_{ij}^{(c)}\big)t^{(d)}}\Big\}\cdot \prod_{i \in \mathcal{A}^{(c)}}\prod_{j\in \mathcal{A}^{(c)}}\frac{(\lambda_{ij}^{(c)}t^{(d)})^{n^{(ij)}}}{n^{(ij)}!}\Big]\Big[\prod_{m=1}^{M^{(d)}}
    \frac{M^{CK}_{cz_m^{(d)}, \backslash d, m}+\alpha m_k}{\sum_{k=1}^KM^{CK}_{ck, \backslash d, m}+\alpha}\Big]
   \end{aligned}
   \end{equation}
  	with $\lambda_{ij}^{(c)}=\mbox{exp}\Big\{\boldsymbol{\beta}^{(c)T}\boldsymbol{x}^*_t(i, j)\Big\}\cdot 1\{j \in \mathcal{A}^{(c^{(d)})}\}$. Moreover, since our $n^{(ij)}$ is either 0 or 1, Equation (8) can be simplified as below:
  	\begin{equation} \gamma_{c}\cdot \mbox{exp}\Big\{{-\big(\sum\limits_{i \in  \mathcal{A}^{(c)}}\sum\limits_{j\in \mathcal{A}^{(c)}}\lambda_{ij}^{(c)}\big)t^{(d)}}\Big\}\cdot \lambda_{i^{(d)}j^{(d)}}^{(c)}t^{(d)}\cdot \Big[\prod_{m=1}^{M^{(d)}}
  	\frac{M^{CK}_{cz_m^{(d)}, \backslash d, m}+\alpha m_k}{\sum_{k=1}^KM^{CK}_{ck, \backslash d, m}+\alpha}\Big]
  	\end{equation}
  	Furthermore, we can speed up the computation time by taking the log of Equation (9), which becomes:
  	 \begin{equation}
  \mbox{log}(\gamma_{c})-t^{(d)}\big(\sum\limits_{i \in \mathcal{A}^{(c)}}\sum\limits_{j\in \mathcal{A}^{(c)}}\lambda_{ij}^{(c)}\big)+\mbox{log}(\lambda_{i^{(d)}j^{(d)}}^{(c)}t^{(d)})+\sum_{m=1}^{M^{(d)}}\mbox{log}(\frac{M^{CK}_{cz_m^{(d)}, \backslash d, m}+\alpha m_k}{\sum_{k=1}^KM^{CK}_{ck, \backslash d, m}+\alpha})
  	 \end{equation}
\subsubsection{Resampling $\mathcal{Z}$}
Next, the new values of $z^{(d)}_m$ are sampled using the conditional posterior probability of being topic $k$ as we derived in APPENDIX B:
\begin{equation}
\begin{aligned} & 
...
\prod_{d=1}^D\Big(\mbox{exp}\Big\{{-\big(\sum\limits_{i \in A^{(c^{(d)})}}\sum\limits_{j\in \mathcal{A}^{(c^{(d)})}}\lambda_{ij}^{(c^{(d)})}\big)t^{(d)}}\Big\}\cdot \prod_{i \in A^{(c^{(d)})}}\prod_{j\in \mathcal{A}^{(c^{(d)})}}\frac{(\lambda_{ij}^{(c^{(d)})}t^{(d)})^{n_d^{(ij)}}}{n_d^{(ij)}!}\Big).
\end{equation}
For $P(\mathcal{B})$, we select a multivarate Gaussian priors as mentioned earlier. Again, since our $n^{(ij)}$ is either 0 or 1, we can use the simplified equation as in Equatin (9) and also can take the log for faster computation as following:
\begin{equation}
\begin{aligned} 
&\mbox{log}(P(\mathcal{B'}))-\mbox{log}(P(\mathcal{B}))\\&\quad \quad+\sum_{d=1}^D\Big(-t^{(d)}\Big(\sum\limits_{i \in A^{(c^{(d)})}}\sum\limits_{j\in \mathcal{A}^{(c^{(d)})}}\lambda_{ij}^{'(c^{(d)})}\Big)+\mbox{log}(\lambda_{i^{(d)}j^{(d)}}^{'(c^{(d)})}t^{(d)})\Big)\\&\quad\quad-\sum_{d=1}^D\Big(-t^{(d)}\Big(\sum\limits_{i \in A^{(c^{(d)})}}\sum\limits_{j\in \mathcal{A}^{(c^{(d)})}}\lambda_{ij}^{(c^{(d)})}\Big)+\mbox{log}(\lambda_{i^{(d)}j^{(d)}}^{(c^{(d)})}t^{(d)})\Big)\\&
=\mbox{log}(P(\mathcal{B'}))-\mbox{log}(P(\mathcal{B}))\\&-\sum_{d=1}^D\Big(t^{(d)}\Big(\sum\limits_{i \in A^{(c^{(d)})}}\sum\limits_{j\in \mathcal{A}^{(c^{(d)})}}\Big(\mbox{exp}\Big\{\boldsymbol{\beta'}^{(c^{(d)})T}\boldsymbol{x}^*_{t^{(d)}}(i, j)\Big\}-\mbox{exp}\Big\{\boldsymbol{\beta}^{(c^{(d)})T}\boldsymbol{x}^*_{t^{(d)}}(i, j)\Big\}\Big)\cdot 1\{j \in \mathcal{A}^{(c^{(d)})}\}\Big)\\&\quad \quad+\Big(\boldsymbol{\beta'}^{(c^{(d)})T}-\boldsymbol{\beta}^{(c^{(d)})T}\Big)\boldsymbol{x}^*_{t^{(d)}}(i^{(d)}, j^{(d)})
\end{aligned}
\end{equation}
Then the log of the acceptance ratio we have is:
\begin{equation}
\mbox{log(Acceptance Probability) = min((17), 0) }
\end{equation}
...
\\&= \Big[\prod_{d=1}^{D}\prod_{m=1}^{M^{(d)}} P( w_m^{(d)}| \phi_{z_m^{(d)}})\Big] \Big[\prod_{d=1}^{D}\prod_{m=1}^{M^{(d)}} P( z_m^{(d)}| \boldsymbol{\theta}^{(c)})\Big] \Big[\prod_{d=1}^{D} P( \mathbf{N}^{(d)}(t^{(d)})| c^{(d)}, \boldsymbol{x}(t^{(d)}), \boldsymbol{\beta}^{(c)})\Big]  \\&\quad \quad \Big[\prod_{c=1}^{C} P( \boldsymbol{\beta}^{(c)}| \sigma^2)\Big]\Big[\prod_{k=1}^{K} P( \boldsymbol{\phi}^{(k)}| \delta, \boldsymbol{n})\Big] \Big[\prod_{c=1}^{C} P( \boldsymbol{\theta}^{(c)}|\alpha, \boldsymbol{m})\Big] \Big[\prod_{d=1}^{D} P(c^{(d)}|\boldsymbol{\gamma})\Big]  P(\boldsymbol{\gamma}|\boldsymbol{\eta})
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
& \propto \Big[\prod_{d=1}^{D}\prod_{m=1}^{M^{(d)}} P( w_m^{(d)}| \phi_{z_m^{(d)}})\Big] \Big[\prod_{d=1}^{D}\prod_{m=1}^{M^{(d)}} P( z_m^{(d)}| \boldsymbol{\theta}^{(c)})\Big] \Big[\prod_{d=1}^{D} P( \mathbf{N}^{(d)}(t^{(d)})|   c^{(d)}, \boldsymbol{x}(t^{(d)}), \boldsymbol{\beta}^{(c)})\Big]\\& \quad \quad  \Big[\prod_{k=1}^{K} P( \boldsymbol{\phi}^{(k)}| \delta, \boldsymbol{n})\Big] \Big[\prod_{c=1}^{C} P( \boldsymbol{\theta}^{(c)}|\alpha, \boldsymbol{m})\Big] \Big[\prod_{d=1}^{D} P(c^{(d)}|\boldsymbol{\gamma})\Big] \\&
= \Big[\prod_{d=1}^{D}\prod_{m=1}^{M^{(d)}} \phi_{w_m^{(d)}z_m^{(d)}}\Big] \Big[\prod_{d=1}^{D}\prod_{m=1}^{M^{(d)}} \boldsymbol{\theta}^{(c)}_{z_m^{(d)}}\Big]\Big[\prod_{d=1}^{D} \Big(\mbox{exp}\Big\{{-\big(\sum\limits_{i \in \mathcal{A}^{(c^{(d)})}}\sum\limits_{j\in \mathcal{A}^{(c^{(d)})}}\lambda_{ij}^{(c^{(d)})}\big)t^{(d)}}\Big\}\prod_{i \in  \mathcal{A}^{(c^{(d)})}}\prod_{j\in \mathcal{A}^{(c^{(d)})}}\frac{(\lambda_{ij}^{(c^{(d)})}t^{(d)})^{n^{(ij)}}}{n^{(ij)}!}\Big)\Big]\\& \quad \quad  \Big[\prod_{k=1}^{K} \Big(\frac{\Gamma(\sum_{w=1}^{W}\delta n_w)}{\prod_{w=1}^{W}\Gamma(\delta n_w)}\prod_{w=1}^{W}\phi_{wk}^{\delta n_w-1} \Big)\Big] \Big[\prod_{c=1}^{C} \Big(\frac{\Gamma(\sum_{k=1}^{K}\alpha m_k)}{\prod_{k=1}^{K}\Gamma(\alpha m_k)}\prod_{k=1}^{K}(\boldsymbol{\theta}^{(c)}_{k})^{\alpha m_k-1} \Big)\Big] \Big[\prod_{d=1}^{D} \gamma_{c}^{I(c^{(d)}=c)}\Big] \\&
=\Big[\frac{\Gamma(\sum_{w=1}^{W}\delta n_w)}{\prod_{w=1}^{W}\Gamma(\delta n_w)}\Big]^K \Big[\frac{\Gamma(\sum_{w=1}^{W}\delta n_w)}{\prod_{w=1}^{W}\Gamma(\delta n_w)}\Big]^C \Big[\prod_{d=1}^{D} \Big(\mbox{exp}\Big\{{-\big(\sum\limits_{i \in \mathcal{A}^{(c^{(d)})}}\sum\limits_{j\in \mathcal{A}^{(c^{(d)})}}\lambda_{ij}^{(c^{(d)})}\big)t^{(d)}}\Big\}\cdot\lambda_{i^{(d)}j^{(d)}}^{(c^{(d)})}t^{(d)}\Big)\Big]\Big[\prod_{d=1}^{D}\gamma_{c^{(d)}}\Big]\\& \quad \quad 
\Big[\prod_{k=1}^{K}\prod_{w=1}^{W}\phi_{wk}^{M^{WK}_{wk}+\delta n_w-1}\Big]\Big[\prod_{c=1}^{C}\prod_{k=1}^{K}(\boldsymbol{\theta}^{(c)}_{k})^{M^{CK}_{ck}+\alpha m_k-1}\Big]
\end{aligned}
\end{equation}
...
\\&=\mbox{Const.}\Big[\prod_{k=1}^{K}\int_{\phi_{:k}}\prod_{w=1}^{W}\phi_{wk}^{M^{WK}_{wk}+\delta n_w-1  }d\phi_{:k}\Big]\Big[\prod_{c=1}^{C}\int_{\theta_{:c}}\prod_{k=1}^{K}(\boldsymbol{\theta}^{(c)}_{k})^{M^{CK}_{ck}+\alpha m_k-1}d\theta_{:c}\Big]
\\&=\mbox{Const.}\Big[\prod_{k=1}^{K}\frac{\prod_{w=1}^W\Gamma(M_{wk}^{WK}+\delta n_w)}{\Gamma(\sum_{w=1}^WM_{wk}^{WK}+\delta )}\Big]\Big[\prod_{c=1}^{C}\frac{\prod_{k=1}^K\Gamma(M^{CK}_{ck}+\alpha m_k)}{\Gamma(\sum_{k=1}^KM^{CK}_{ck}+\alpha)}\Big].
\end{aligned}
\end{equation}
...
\frac{P(\mathcal{W}, \mathcal{Z}, \mathcal{C}|\delta, \boldsymbol{n}, \alpha, \boldsymbol{m})}{P(\mathcal{W}_{\backslash d, m}, \mathcal{Z}_{\backslash d, m}, \mathcal{C}|\delta, \boldsymbol{n}, \alpha, \boldsymbol{m})}\\& \propto \frac{\prod_{k=1}^{K}\frac{\prod_{w=1}^W\Gamma(M_{wk}^{WK}+\delta n_w)}{\Gamma(\sum_{w=1}^WM_{wk}^{WK}+\delta )}\prod_{c=1}^{C}\frac{\prod_{k=1}^K\Gamma(M^{CK}_{ck}+\alpha m_k)}{\Gamma(\sum_{k=1}^KM^{CK}_{ck}+\alpha)}}{\prod_{k=1}^{K}\frac{\prod_{w=1}^W\Gamma(M_{wk, \backslash d, m}^{WK}+\delta n_w)}{\Gamma(\sum_{w=1}^WM_{wk, \backslash d, m}^{WK}+\delta )}\prod_{c=1}^{C}\frac{\prod_{k=1}^K\Gamma(M^{CK}_{ck, \backslash d, m}+\alpha m_k)}{\Gamma(\sum_{k=1}^KM^{CK}_{ck, \backslash d, m}+\alpha)}}\\ & \propto
\frac{M_{wk, \backslash d, m}^{WK}+\delta n_w}{\sum_{w=1}^WM_{wk,  \backslash d, m}^{WK}+\delta}\frac{M^{CK}_{ck, \backslash d, m}+\alpha m_k}{\sum_{k=1}^KM^{CK}_{ck, \backslash d, m}+\alpha}
\end{aligned}
\end{equation}
