...
for(i in 1:nodes){
  ...
  for(j in (1:nodes)[-i]){
    pij <- 1/(1+exp(-rbind(netstats[d,i,j,])%*%cbind(beta)))
...
for(i in 1:nodes){
  netstatsi <- rbind(netstats[d,i,which(adjacency_matrix[i,]==1),])
...
simulation_result_file <- paste("./results/DareSimulatedDocumentsD",d,".RData",sep="")
