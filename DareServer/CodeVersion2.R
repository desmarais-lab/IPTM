...
#3-2. sortedZ: sort the documents and corresponding topics according to the IPs
cppFunction('
...
MCMC =function(edge, node, textlist, vocabulary, nIP, K, delta_B, outer=200, n1=3, n2=3, n3=3300, burn=300, thin=3, seed=1){
	...
	theta<- lapply(1:nrow(edge), function(d){rdirichlet(1,alpha[[currentC[d]]]*mvec[[currentC[d]]])})					
     ...
     for (i1 in 1:n1){
	    ...
	    for (d in 1:nrow(edge)){
			...
			for (IP in 1:nIP){
	       	...
	       	const<-log(gammas)+betapartC(nIP, lambdai, edge[d,])+topicpartC(nIP, K, currentZ, alpha, mvec, d)
