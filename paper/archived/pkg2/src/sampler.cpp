	...
	if (iter > 0) {
	  for (int i = 0; i < iter; i++) {
	    ...
	    for (int r = 0; r < receiver.size(); r++){
  			  if (time >= time3) {
  			  		NumericMatrix IP_l = IPmat[0];
				    IP_l(sender - 1, receiver[r] -1) += 1;
				    IPmat[0] = IP_l;
			    }
  			  if (time >= time2 && time < time3) {
  				 	NumericMatrix IP_l = IPmat[1];
				    IP_l(sender - 1, receiver[r] -1) += 1;
				    IPmat[1] = IP_l;
			    }  				
			    if (time >= time1 && time < time2) {
  			  		NumericMatrix IP_l = IPmat[2];
				    IP_l(sender - 1, receiver[r] -1) += 1;
				    IPmat[2] = IP_l;
			    } 		
  			}
...
NumericMatrix Degree(List history, IntegerVector node, int sender) {
   ...
   for (int b = 0; b < node.size(); b++) {
    int receiver = node[b];
    for (int l = 0; l < 3; l++) {
    	NumericMatrix historyIP_l = history[l];
  ...
  NumericMatrix IPmat = degreemat_IP;
  return IPmat;
}
...
NumericMatrix Dyadic(List history, IntegerVector node, int sender) {
    ...
    NumericMatrix IPmat = dyadicmat_IP; 
  return IPmat;
}  
...
NumericMatrix Triadic(List history, IntegerVector node, int sender) {
      NumericMatrix triadmat_IP(node.size(), 36);
        ...
        for (int l = 0; l < 3; l++) {
          for (int m = 0; m < 3; m++){
            NumericMatrix historyIP_l = history[l];
            NumericMatrix historyIP_m = history[m];
       	    for (int h = 0; h < node.size(); h++) {
     	        int third = node[h];	
      ...
      NumericMatrix IPmat = triadmat_IP;
  return IPmat;
}
...
NumericMatrix Triadic_reduced(NumericMatrix triadic) {
   	...
   	NumericMatrix historyIP = triadic;
