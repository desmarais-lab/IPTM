...
Daretest <- IPTM_inference.data(Dare$edge, Dare$node, Dare$text, Dare$vocab, nIP = 2, K = 50, sigma_Q = c(0.01, 1),
alpha = 2, mvec = rep(1/50, 50), betas = 2, nvec = rep(1/length(Dare$vocab), length(Dare$vocab)), prior.b.mean = c(-1, rep(0, 6)),
...
save(Daretest, file = "Daretest_IPTM_K50.RData")
