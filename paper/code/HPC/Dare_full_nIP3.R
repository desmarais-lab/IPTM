...
Daretest <- IPTM_inference.data(Dare$edge, Dare$node, Dare$text, Dare$vocab, nIP = 3, K = 20, sigma_Q = c(0.0000005, 0.01),
alpha = 2, mvec = rep(1/20, 20), betas = 2, nvec = rep(1/length(Dare$vocab), length(Dare$vocab)),
                                prior.b.mean = rep(0, 25),
prior.b.var = diag(25), prior.delta = c(0, 1), out = 100, n_B = 5500, n_d = 550, burn = c(500, 50),
thinning = c(10, 1),netstat = c("intercept", "dyadic", "degree", "triadic"), optimize = TRUE)
save(Daretest, file = "Daretest_IPTM_K20_nIP3.RData")
