...
par(mfrow = c(1,3))
plot(colnames(table_sum), table_sum[1,], type = 'l', col = 'red', xlab = "Number of Topics", lty = 2, ylab = "Sender AUC", ylim = c(table_sum[1,1] - 0.02, table_sum[1,1] + 0.02))
lines(colnames(table_sum), all[1:7,1], lty = 3,col = "blue")
lines(colnames(table_sum), all[8:14,1], lty = 4,col = "green")
lines(colnames(table_sum), all[15:21,1], lty = 5,col = "purple")
legend(60, 0.545, pch = 21, legend = c("IPTM with C = 3","IPTM with C = 2","IPTM with C = 1", "Regressions"), col = c("purple", "green", "blue", "red"))

plot(colnames(table_sum), table_sum[2,], type = 'l', col = 'red', xlab = "Number of Topics", lty = 2, ylab = "Receiver F1Score", ylim = c(table_sum[2,1] - 0.02, table_sum[2,1] + 0.01))
lines(colnames(table_sum), all[1:7,2], lty = 3,col = "blue")
lines(colnames(table_sum), all[8:14,2], lty = 4,col = "green")
lines(colnames(table_sum), all[15:21,2], lty = 5,col = "purple")
legend(60, 0.96, pch = 21, legend = c("IPTM with C = 3","IPTM with C = 2","IPTM with C = 1", "Regressions"), col = c("purple", "green", "blue", "red"))

plot(colnames(table_sum), table_sum[3,], type = 'l', col = 'red', xlab = "Number of Topics", lty = 2, ylab = "Time MSE", ylim = c(0, 1.6))
lines(colnames(table_sum), all[1:7,3], lty = 3,col = "blue")
lines(colnames(table_sum), all[8:14,3], lty = 4,col = "green")
lines(colnames(table_sum), all[15:21,3], lty = 5,col = "purple")
legend(60, 1.55, pch = 21, legend = c("IPTM with C = 3","IPTM with C = 2","IPTM with C = 1", "Regressions"), col = c("purple", "green", "blue", "red"))
...
for (d in 1:22) {
  ...
  observeds = tabulate(Dare$edge[[D]]$sender, 27)
  observedr = tabulate(Dare$edge[[D]]$receiver, 27)
  observedt = Dare$edge[[D]]$timestamp
  tables[[d]] = matrix(0, nrow = 21, ncol = 3)
  rownames(tables[[d]]) = 1:21
  iter = 1
  for (nIP in 1:3) {
    ...
    for (o in 1:10) {
      for (r in 1:50) {
        ...
        F1score = F1score + F1_Score(observedr, tabulate(PPE[[o]][[r]]$edge$receiver, 27))
        MSE = MSE + (PPE[[o]][[r]]$edge$timestamp - observedt)^2
          it = it + 1
          }
      }
...
par(mfrow = c(1,3))
plot(colnames(table_sum), table_sum[1,], type = 'l', col = 'red', xlab = "Document", lty = 2, ylab = "Sender AUC", main = "Enron")
plot(colnames(table_sum), table_sum[2,], type = 'l', col = 'red', xlab = "Document", lty = 2, ylab = "Receiver F1Score", main = "Enron")
plot(colnames(table_sum), table_sum[3,], type = 'l', col = 'red', xlab = "Document", lty = 2, ylab = "Time MSE", main = "Enron")
