...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.98\textwidth]{plots/Schein_C_50K.pdf} 
	\label{fig:PPplot_C}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.98\textwidth]{plots/Schein_noC_50K.pdf} 
...
The next step in our work will be to either repair any bugs that exist in our $\mathcal{C}$ inference code, so we currently do not infer topic-interaction pattern assignment when applying to real-world data, in the next section.
 ...
 To see the applicability of the model, we used the North Carolina email data using two counties, Vance county and Dare county, which are the two counties whose email corpus cover the date of Hurricane Sandy (October 26, 2012 – October 30, 2012). Especially, Dare county geographically covers the Outer Banks (See Figure 4), one of the most critically affected area by the hurricane, thus we expect to see how the communication pattern changes during the time period surrounding Hurricane Sandy. In this section we apply IPTM to both county data and demonstrate the effectiveness of the model at predicting and explaining continuous-time textual communications.
 \subsection{Exploratory Data Analysis} \label{subsec: EDA}
 \subsubsection{Vance County}\label{subsubsec:VanceEDA}
 We conducted some expoloratory data analysis on Vance county email dataset, where the data spans September 4th to November 30th, containing $D=183$ emails sent between $A=17$ actors with $W=620$ vocabularies in total. 
 \begin{figure}[ht]
 	\centering
 	\includegraphics[width=0.49\textwidth]{plots/Vance.png} 
 	\label{fig:Vance}
 	\caption{Geographical location of Vance county in North Carolina}
 \end{figure}
 
 First, we explored the temporal change in sending and receiving behavior of government managers, based on their departments.
  \begin{figure}[ht]
  	\centering
  	...
  	\caption{The number of emails sent (left) and received (right) for thecounty managers in each department in Vance county}
  \end{figure}
 ...
 Network plots, which are drawn upon 3 week period, 
   ...
   \end{figure}
   \subsubsection{Dare County}\subsubsection{DareEDA}
...
<<<<<<< HEAD
=======
        \subsubsection{GiR Traceplot} \label{subsubsec: GiR Traceplot}
>>>>>>> origin/master
  \end{document}
