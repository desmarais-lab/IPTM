...
The Vance County email dataset spans September 4th to November 30th. It is also the smallest corpus in the NC overall dataset, containing $D=183$ emails sent between $A=17$ actors from 17 departments. The corpus has a vocabulary size of $W=620$. %[bd, here] 
Our preliminary analysis was focused on comparing the email exchange patterns between non-Sandy period (not overlapping October 26---30) and Sandy period (October 26---30). 
 ...
 In the similar context, we also created the network plots for three separate time windows: pre-Sandy, Sandy, and post-Sandy. Sandy period includes October 19th to November 2nd, which covers the one week before and after the official period of Sandy. Figure 6 showed similar pattern as Figure 5, in that the same three department managers are placed in the center of each network. One noticeable thing is that during Sandy period, social services did not exchange any email while it went back to normal status after Sandy. In addition, emergency service department slightly interacted more actively during Sandy, compared to the other two time windows.
   ...
   \subsection{IPTM Results} \label{subsec: IPTM results}
          ...
          In general, Figure 11 and Table 1 both do not show any strong difference between the two interaction patterns. When we look at the network effects (See Figure 11), the two boxplots representing each interaction pattern mostly overlap, although the point estimates for few statistics had different signs (e.g. outdegree, 2-receive and sibling). Moreover, the 95\% credible intervals for most of statistics, except the intercept, include 0, thus we do not find any significant effect of past interactions on future email exchanges. Although \cite{PerryWolfe2012} found out significant recency effects in email interactions, in our cases, there is no distinctive recency effect since we do not see any increasing or decreasing trends over the three time intervals. 
          \begin{table}[ht]
          	\centering
          	\begin{tabular}{lll}
          ...
          \begin{table}[ht]
          	...
          	This time, the two interaction patterns showed different structures in terms of networks, since the credible interval of many of network effects do not overlap. Interaction pattern 2 had more negative network effects while having much larger intercept compared to interaction pattern 1, implying that topics related to interaction pattern 2 are less affected by previous email exchanges.
          	\begin{table}[ht]
          		\centering
          		\begin{tabular}{lll}
          	...
          	There also exists significant difference in the contents related to each interaction pattern. Overall, 55.2\% of the words were assigned to the topics in interaction pattern 1, on the other hand, 44.8\% were assigned to the topics in interaction pattern 2. Table 4 demonstrates 5 examples of topics from each interaction pattern, where each topic is summarized by top 15 words. It is interesting that interaction pattern 2 included several topics (e.g. topic 2 and topic 18) with hurricane related words, such as `storm', `impacts' `damage' and `ocean'. On the other hand, interaction pattern 1 seems to reperesent topics commonly used between government managers.
          	\begin{table}[ht]
          		\centering
          		...
          		We further look at the topic-token assignments on the subset of text that only includes documents during hurricane Sandy period, October 19---November 2 (from one week before Sandy to one week after Sandy) , which was used to generate the network plot for Sandy period in Figure 6 and Figure 9. This time, the we see more storm-related words in interaction pattern 2, thus the difference between two interaction patterns are more distinctive.
          		     	\begin{table}[ht]
          		     		\centering
          		     		\scalebox{0.8}{	 	\begin{tabular}{ |c||c|c|c|c|c|}  
          		     				\hline
          		     				\textbf{IP} & \textbf{1} &  \textbf{1} & \textbf{1}  &\textbf{1} &\textbf{1}   \\ \hline\hline
          		     				\textbf{Topic} & \textbf{3} (0.090)&  \textbf{5} (0.090)& \textbf{13} (0.068) &\textbf{7} (0.056) &\textbf{19} (0.053)  \\ \hline\hline
          		     				\textbf{Word} 
          		     				& water & planning & collins & hours & manager \\
          		     				& department & meter & marshall & time & law\\
          		     				& hydrant &needed & phone & leave & manteo\\
          		     				& skyco & asked & director & monday & public\\
          		     				& road & sure & manteo & employees & department\\
          		     				& relocation & cheryl & drive & day & email\\
          		     				& location & johnson & human & administrative & carolina\\
          		     				& kill & fema & box & october & talked\\
          		     				& deivil & keeping & phr & employee & correspondence\\
          		     				& hills & afternoon & resources & work & subject\\
          		     				& utilities & -lsb- & fax & regular & box\\
          		     				& mustian & room & -lsb- & hour& north\\
          		     				& lane & case & personnel & scheduled & contract\\
          		     				& soundside & unsafe & touch & friday & attorney\\
          		     				& system & northeast & wanted & benefits & state \\
          		     				\hline \hline
          		     				\textbf{IP} & \textbf{2} &  \textbf{2} & \textbf{2}  &\textbf{2} &\textbf{2}   \\ \hline\hline
          		     				\textbf{Topic} & \textbf{12} (0.062)&  \textbf{16} (0.042)& \textbf{18} (0.041) &\textbf{8} (0.037) &\textbf{10} (0.034) \\ \hline\hline
          		     				\textbf{Word}
          		     				& point & high & number & center & system \\
          		     				& weekend & impacts & report & moving & forecast \\
          		     				& change & saturday& office & radios & track \\
          		     				& strong & night & cell & shore & south \\
          		     				& potential & winds & finance & site & area\\
          		     				& low/tc & bridge& project & expect & well\\
          		     				& days & normal & fax & reports & opens\\
          		     				&nanofiltration & storm & going & banks &  move \\
          		     				& dss & dot & emergency & continues & obx \\
          		     				& ordinances & sunday & well & november & operation \\
          		     				& commissioners & head & exam & noon  & damage \\
          		     				& sandy& gale & discuss & clarification/question & pay \\
          		     				& oct & expressed & account & latest & north \\
          		     				& unis &nags & 're & equipment & assessment \\
          		     				& car & outer & hurricane & aware & ocean 
          		     				\\
          		     				\hline            				
          		     			\end{tabular}}
          		     			\label{table:TableDare2}
          		     			\caption{Summary of topic-token assignments from Dare county data: top 15 words assigned to each topic, corresponding to interaction pattern assignments}               			
          		     		\end{table} 
  \newpage
