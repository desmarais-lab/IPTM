...
To examine shifts in the patterns of connectivity surrounding Sandy we created network plots for three separate time windows: pre-Sandy, Sandy, and post-Sandy. The Sandy period includes October 19th to November 2nd, which covers the one week before and after the official period of Sandy. Figure 6 shows a similar pattern as that seen in Figure 5, in that the same three department managers are placed in the center of each network. One noticeable thing is that during the Sandy period, social services did not exchange any email, while it went back to its normal status after Sandy. In addition, the emergency service department interacted slightly more during Sandy, compared to the other two time windows.
   ...
   In this section we present the IPTM results. Researchers who use the IPTM can test hypotheses regarding network structure, which is a common---perhaps the most common---use of ERGM-stye models for networks. However, with the IPTM those hypotheses can be conditioned on the content area of communication. To provide an example, we articulate expectations regarding the content-conditional structural properties of the county government email networks. Information diffuses more efficiently in networks characterized by a lack of loops \citep{lin2010measuring,iribarren2011branching} and closed triangles \citep{roca2010topological,tadic2004information}. Assuming that the county governments' internal communication networks are characterized by efficient communication structure,  we expect to see communication regarding the everyday business of the county characterized by negative reciprocity (i.e., 2-loop) effects, and negative triadic effects. The ``receive'' term captures reciprocity, and triadic effects are captured by 2-send, 2-receive, sibling and cosibling. Reciprocity and closed triangles are, however, common structural properties of social networks. We expect to see communication regarding personal/social matters to be characterized by positive reciprocity and triadic effects. Lastly, we have limited expectations regarding how communication surrounding Hurricane Sandy will be structured. We take an exploratory approach to the question of whether or not discussion surrounding Sandy forms an efficient communication network structure.
         ...
         In the IPTM analysis of the Vance County data we set $C=2$, $K=6$ and $O= 500$. Adding more interaction patterns and/or topics did not appear to change or improve results significantly, but in future work we plan to develop principled methods for selecting these hyperparameters. MCMC sampling was implemented based on the scheme illustrated in Section \ref{sec: Inference}. We applied hyperparameter optimization with $n_1=5$, and the inner iterations for $\mathcal{B}$ and $\delta$ were set as $n_2=5500$ and $n_3 = 550$, respectively. First $500$ and $50$ iterations were discarded as a burn-in for inference on  $\mathcal{B}$ and $\delta$, and every $10^{th}$ sample was taken in order to thin the chain for $\mathcal{B}$. Below are the summary of interaction pattern-topic-word assignments. %[bd, here]
          Each interaction pattern is paired with (a) Table 1: the top 15 most likely words to be generated conditioned on the topic and their corresponding interaction pattern, and (b) Figure 11: posterior estimates of dynamic network effects $\boldsymbol{b}^{(c)}$ corresponding to the interaction pattern
          
          In general, Table 1 and Figure 11 both do not show any strong difference between the two interaction patterns. Topic-token assignments for two interaction patterns in Table 1 also do not reveal obvious contrast, because the topics all contain words used for typical conversations between government officials. 
               \begin{table}[ht]
               	\centering
          ...
          \end{table}
          	...
          	Same as Section \ref{subsubsec:DareIPTM}, we present the two model results representing the network aspects and contents. Each interaction pattern is paired with (a) Table 2: the top 15 most likely words to be generated conditioned on the topic and their corresponding interaction pattern, and (b)   Figure 12: posterior estimates of dynamic network effects $\boldsymbol{b}^{(c)}$ corresponding to the interaction pattern.
          	
          		There exists significant difference in the contents related to each interaction pattern. Overall, 55.2\% of the words were assigned to the topics in interaction pattern 1, on the other hand, 44.8\% were assigned to the topics in interaction pattern 2. Table 4 demonstrates 5 examples of topics from each interaction pattern, where each topic is summarized by top 15 words. Similarly as what we saw in Vance County, interaction pattern 1 seems to reperesent topics commonly used between government managers. On the other hand, it is interesting that interaction pattern 2 included several topics (e.g. topic 2 and topic 18) with hurricane related words, such as `storm', `impacts' `damage' and `ocean'. In addition, one more impressive point is that topic 12 contained words related politics or election, e.g. `survey', `parties', and `elections'. Therefore, we can summarize the interaction pattern 1 to represent usual communications between the managers in county government, and the interaction pattern 2 to reflect some of temporary conversations contingent upon the events or emergencies occurring in the County.
          		\begin{table}[ht]
          			\centering
          	...
          	This time, the two interaction patterns showed quite different structures in terms of networks. Interaction pattern 1 had more positive or negative significant network effects while having much lower intercept compared to interaction pattern 2, implying that topics related to interaction pattern 1 are less affected by previous email exchanges. There also exists strong recency effect, i.e. recent interactions are more influential compared to older interactions, which can be found in many statistics including `outdegree
          	, `indegree', `2-receive',  `sibling' , and `cosibling'. They have larger effects for the time interval $[t-24h, t)$ and the effect is diminishing as we move to older time intervals $[t-96h, t-24h)$ and $[t-384h, t-96h)$. On the contrary, the statistic `send' had strongest effect from the oldest history $[t-384h, t-96h)$ while the recent ones are not significant, which suggest that if $i$ sent an email to $j$ earlier between 3 days and 16 days ago, $i$ will highly likely to send email to $j$ again, while if the previous email was sent within last 24 hours, $i$ has no reason for sending email to $j$ again. Furthermore, negative reciprocity, which was expected in the introductions of Section \ref{subsec: IPTM results}, are found in `receive' statistics effect (except $[t-96h, t-24h)$), indicating efficient comunication structure regarding the daily county businesses. Although interaction pattern 1 contains many interesting stories, interaction pattern 2 does not seems to have any significant network effects, because the 95\% credible intervals for all network statistics include 0. This can be explained by the word aspects we found in Table 3. Since interaction pattern 2 consists of temporal topics, thus it is less likely to be effected by previous interactions, instead, county managers may discuss these event or emergency-related topics purely based on the topical relevance.
          	\begin{table}[ht]
          		\centering
          		\begin{tabular}{lll}
          	...
          	\end{table}
 \iffalse
          {\bf Not sure if this should be included}
