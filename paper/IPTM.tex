...
\newpage
 ...
 \begin{algorithm}[H]
	...
	\For{o=1 to O}{
		...
		\For{r=1 to R}{
		initialize latent receivers $\{J_i^{D^*}\}_{i=1}^A$ and $\mathcal{Z}^{D^*}$ using empirical estimates from the infernece\\
		update $\{J_i^{D^*}\}_{i=1}^A$ using Gibbs update -- Equation (15) without edge information\\
		update $\mathcal{Z}^{D^*}$ using Gibbs update -- Equation (24) without edge information \\
		sample  $i^{(D^*)}$, $J^{(D^*)}$, and $t^{(D^*)}$ conditional on $\{J_i^{D^*}\}_{i=1}^A$ and $\mathcal{Z}^{D^*}$, via the generative process
		}
		store $i^{(D^*)}$, $J^{(D^*)}$, $t^{(D^*)}$, and $\mathcal{Z}^{D^*}$
	}
\end{algorithm}
