...
The next step in our work will be to either repair any bugs that exist in our $\mathcal{C}$ inference code, so we currently do not infer topic-interaction pattern assignment when applying to real-world data, in the next section.
 ...
 We conducted some expoloratory data analysis on Vance county email dataset, where the data spans October 10 to November 30, containing $D=185$ emails sent between $A=18$ actors with $W=620$ vocabularies in total. 
 \begin{figure}[ht]
 	\centering
...
We calculated 1,000 quantiles for each of the network effect statistics (1.), and 50 quantiles for the rest of the statistics. We also calculated a t-test and Mann-Whitney test p-values for the equivalence of statistic means between forward and backward samples. Before we calculated these statistics, we first thinned our sample of
...
>>>>>>> origin/master
 ...
 First, we explored the temporal change in sending and receiving behavior of government managers, based on their departments.
  \begin{figure}[ht]
  	\centering
  	  	\includegraphics[width=0.44\textwidth]{plots/Sendplot.pdf} 
  	  	\includegraphics[width=0.44\textwidth]{plots/Receiveplot.pdf} 
  	  	  	  	\includegraphics[width=0.1\textwidth]{plots/Dept2.jpeg} 
 ...
 Network plots, which are drawn upon 3 week period, 
   \begin{figure}[ht]
   	\centering
   	\includegraphics[width=0.8\textwidth]{plots/Networkplot.pdf} 
   	  	  	  	  	\includegraphics[width=0.1\textwidth]{plots/Dept2.jpeg} 
   ...
   \subsubsection{Dare County}\subsubsection{DareEDA}
    and Dare county data spans October 1 to November 30 containing $D=1456$ emails between $A=27$ actor with $W=2907$ vocabularies in total
     \begin{figure}[ht]
     	\centering
