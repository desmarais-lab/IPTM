 ...
 To actually perform inference, we therefore want to use Gibbs sampling, where we will sequentially resample the values of each of our latent variables from their posterior distribution, conditional on all of our other variables.
    \subsection{Resampling the augmented data $\mathcal{J}_a$} \label{subsec: Data augmentation}
      ...
      where $\lambda^{(d)}_{iJ^{(d)}_{i[+j]}}$ meaning that preveiously defined $\lambda^{(d)}_{iJ^{(d)}_{i}}=\sum\limits_{c=1}^{C} p^{(d)}_c\cdot\mbox{exp}\Big\{\frac{1}{|J_i|}\sum\limits_{j \in{J_i}}\boldsymbol{b}^{(c)T}\boldsymbol{x}^{(c)}_{t^{(d-1)}_+}(i, j)\Big\}\cdot \prod\limits_{j \in J_i}1\{j \in \mathcal{A}_{\backslash i}\} $ is calculated with $j^{th}$ element of $J_i^{(d)}$ fixed as 1. On the other hand, 
       ...
       \end{aligned}
       \end{equation}
       where $\lambda^{(d)}_{iJ^{(d)}_{i[-j]}}$ meaning similarly as above that $\lambda^{(d)}_{iJ^{(d)}_{i}}=\sum\limits_{c=1}^{C} p^{(d)}_c\cdot\mbox{exp}\Big\{\frac{1}{|J_i|}\sum\limits_{j \in{J_i}}\boldsymbol{b}^{(c)T}\boldsymbol{x}^{(c)}_{t^{(d-1)}_+}(i, j)\Big\}\cdot \prod\limits_{j \in J_i}1\{j \in \mathcal{A}_{\backslash i}\} $ is calculated with $j^{th}$ element of $J_i^{(d)}$ fixed as 0. Now we can use Gibbs sampling using the two probabilities, Equation (16) and Equation (17). Note that we would calculate the probabilities in the log space to prevent from numerical underflow. Equivalently, for each $i$ and $j$, we can apply a random sampling from the Bernoulli probability
              \begin{equation}
              \begin{aligned}
              &\frac{P(J_{ij}^{(d)}=1|\cdot)}{P(J_{ij}^{(d)}=1|\cdot) + P(J_{ij}^{(d)}=0|\cdot)},
              \end{aligned}
              \end{equation}
              where $P(J_{ij}^{(d)}=1|\cdot)$ is Equation (16) and $P(J_{ij}^{(d)}=0|\cdot)$ is Equation (17).
             \iffalse
...
By multiplying the normal CDF part to Equation (27) and take the log, we obtain the log of acceptance ratio:
 ...
 \end{equation}
\end{comment}
After finishing the updates of $\mathcal{B}$, we move on to the updates of $\delta$. Again, we use the Metropolis-Hastings algorithm with a proposal density $Q$ being the Normal distribution, with variance equals to $\sigma^2_\delta$ (proposal distribution variance parameters set by the user). However, this time we need a proposal distribution with support on the unit interval, since $ \delta \sim \mbox{Beta}(1, 10)$.  Now we define an auxiliary variable $\eta$ which come from the proposal distribution (in the real line) and take the transformation:
 $$\delta = \Phi(\eta),$$
 ...
 \begin{equation} 
 	\begin{split}
 		& \mbox{Acceptance Probability}=
 		\begin{cases}  \frac{P(\delta^\prime|\mathcal{Z},   \mathcal{C},  \mathcal{B}, \mathcal{W}, \mathcal{I}_{\mbox{a}}, \mathcal{J}_{\mbox{a}}, \mathcal{T}_{\mbox{a}}, \mathcal{I}_{\mbox{o}}, \mathcal{J}_{\mbox{o}}, \mathcal{T}_{\mbox{o}}, \beta, \boldsymbol{u}, \alpha, \boldsymbol{m}, \sigma^2)}{P(\delta|\mathcal{Z},   \mathcal{C},   \mathcal{B}, \mathcal{W}, \mathcal{I}_{\mbox{a}}, \mathcal{J}_{\mbox{a}}, \mathcal{T}_{\mbox{a}}, \mathcal{I}_{\mbox{o}}, \mathcal{J}_{\mbox{o}}, \mathcal{T}_{\mbox{o}}, \beta, \boldsymbol{u}, \alpha, \boldsymbol{m}, \sigma^2)}\quad\text{if}  <1\\
 			1 \quad \text{else}
 		\end{cases}
 	\end{split}
 \end{equation}
 ...
 \begin{equation}
 	\begin{aligned} 
 		&\mbox{log}\Big(\mbox{dbeta}(\delta^\prime ;1, 10)\Big)-\mbox{log}\Big(\mbox{dbeta}(\delta;1, 10)\Big)\\&+ \sum_{d=1}^D\Big\{\sum_{i\in \mathcal{A}}\sum_{j \in \mathcal{A}_{\backslash i }} {I(j \in J_i)}\Big(\mbox{log}(\delta^{\prime} \lambda^{(d)}_{iJ_i}) - \mbox{log}(\delta \lambda^{(d)}_{iJ_i})\Big)  {-\Big(\mbox{log}(\delta^{\prime} \lambda^{(d)}_{ij}}+1)
...
\end{comment}
 ...
 \end{algorithm}
   ...
   We need to define a “backward” generative process in order to perform Geweke’s “Getting it Right” test \citep{geweke2004getting} because when we are generating our “backwards” samples, we only want to resample the token word types and (sender, recipients, timestamp) pairs, and not any of
   ...
   Next, we list the selection of statistics we save for each forward and backward sample. Note that these statistics are not sensitive to the label swithches across the updates. Therefore, at each iteration, we calculate and save the statistics below:
      \begin{itemize}
      	...
      	\item[4.] Mean topic-interaction pattern assignment (for interaction patterns indexed by 1 and 2), \textcolor{red}{ ? }
      	\item[5.] Number of tokens in topics assigned to each interaction pattern $c=1,...,C$, \textcolor{red}{?}
      	\item[6.] Number of tokens assigned to each topic $k=1,...,K$, \textcolor{red}{ ? }
       \item[7.] Number of tokens assigned to each unique word type $w=1,...,W$. \textcolor{red}{ ? }
      	      \end{itemize}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=1.05\textwidth]{GiR100000.pdf} 
	\label{fig:GiR}
	\caption{Getting it Right test result}
\end{figure}
\newpage\noindent\textcolor{red}{\textbf{Possible failure reasons}:
	\begin{itemize}
		...
		\item Prior structure (Beta distribution) of $\delta$ seems to be inappropriate. Currently the inference results of $\delta$ do not converge and it always accepts smaller value of $\delta^\prime$. This can lead bias in $B$, ``mean-recipients", and ``mean-timediff"
		\item Tokens in word esitmate seems to be all failure. No idea at all. Need to double check for possible coding errors.
		\item Are the statistics free of label switching issue? Those with ? mark are suspicious.
	\end{itemize}
	}
