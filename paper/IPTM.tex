...
The next step in our work will be to either repair any bugs that exist in our $\mathcal{C}$ inference code, so we currently do not infer topic-interaction pattern assignment when applying to real-world data, in the next section.
 ...
 We first conducted some expoloratory data analysis on Vance county email dataset, where the data spans September 4th to November 30th, containing $D=183$ emails sent between $A=17$ actors with $W=620$ vocabularies in total. To see temporal trends in sending and receiving behavior in email exchanges, we plotted the number of emails sent and received between the county government managers based on their departments, which are shown in Figure 5. 
  \begin{figure}[ht]
  	\centering
  	  	\includegraphics[width=0.49\textwidth]{plots/VanceSend.pdf} 
  	  	\includegraphics[width=0.49\textwidth]{plots/VanceReceive.pdf} 
  	\label{fig:SendReceiveVance}
  	\caption{The number of emails sent (left) and received (right) for the county managers in each department in Vance county}
  \end{figure}
  
 Network plots, which are drawn upon 3 week period, 
   \begin{figure}[ht]
   	\centering
   	\includegraphics[width=0.8\textwidth]{plots/VanceNetwork.pdf}  
   	\label{fig:NetworkVance}
   	\caption{Network plot based on 3 week period, before Sandy, during Sandy, and after Sandy, in Vance county}
   \end{figure}
   
   Lastly, we looked at how many times the word `hurricane' and `sandy' appeared.
      \begin{figure}[ht]
      	\centering
      	\includegraphics[width=0.49\textwidth]{plots/Vanceword.pdf}  
      	\label{fig:WordVance}
      	\caption{Network plot based on 3 week period, before Sandy, during Sandy, and after Sandy, in Vance county}
      \end{figure}
   \subsubsection{Dare County}\label{subsubsec:DareEDA}
    and Dare county data spans September 1st to November 30th containing $D=2210$ emails between $A=27$ actor with $W=2907$ vocabularies in total
        \begin{figure}[ht]
       	\centering
       	\includegraphics[width=0.49\textwidth]{plots/SendDare3.pdf} 
       	\includegraphics[width=0.49\textwidth]{plots/ReceiveDare3.pdf} 
       	\label{fig:SendReceiveDare}
       	\caption{The number of emails sent (left) and received (right) for thecounty managers in each department in Dare county}
       \end{figure}
       
        Network plots, which are drawn upon 3 week period, 
         \begin{figure}[ht]
         	\centering
         	\includegraphics[width=0.8\textwidth]{plots/NetworkDare2.pdf} 
         	\label{fig:NetworkDare}
         	\caption{Geographical location of Vance county and Dare county in North Carolina}
         \end{figure}
         
            Lastly, we looked at how many times the word `hurricane' and `sandy' appeared.
            \begin{figure}[ht]
            	\centering
            	\includegraphics[width=0.49\textwidth]{plots/WordDare.pdf}  
            	\label{fig:WordDare}
            	\caption{Network plot based on 3 week period, before Sandy, during Sandy, and after Sandy, in Vance county}
            \end{figure}
