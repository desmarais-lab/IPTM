...
\begin{itemize} %[bd, here]
	\item[1.] For each sender $i \in \{1,...,A\}$, we create binary receiver vector of length $A-1$, $J^{(d)}_i$, by applying the non-empty Gibbs measure \citep{fellows2017removing} to every $j \in \mathcal{A}_{\backslash i}$, since we exclude self-loop.  
