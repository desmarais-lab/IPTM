...
\newpage
 ...
 \begin{algorithm}[H]
	...
	\For{o=1 to O}{
		...
		initialize $N_{v|k}$ and $N_k$ counts from the inference on $Z$ for documents $1$ through $(D^*-1)$\\
		\For {d = 1 to $(D^*-1)$ } {
		...
		\For{r=1 to b+R}{
		sample  $i_o^{(D^*)}$, $J_o^{(D^*)}$, and $t_o^{(D^*)}$ using the lines of Algorithm 10 beginning with the for loop over $i$ \\
		sample $J_a^{(D^*)}$ using Equations 17 and 18, treating $i^{(D^*)}$, $J^{(D^*)}$, and $t^{(D^*)}$ as the observed edge information for document $D$\\
		sample $\mathcal{Z}^{D^*}$ using Equation 25, treating $i^{(D^*)}$, $J^{(D^*)}$, and $t^{(D^*)}$ as the observed edge information for document $D$ \\
