...
By multiplying the normal CDF part to Equation (27) and take the log, we obtain the log of acceptance ratio:
 ...
 \newpage
 \noindent
 \textcolor{red}{ALTERNATVELY,}\\ After finishing the updates of $\mathcal{B}$, we move on to the updates of $\delta$. Again, we use the Metropolis-Hastings algorithm with a proposal density $Q$ being the Normal distribution, with variance equals to $\sigma^2_\delta$ (proposal distribution variance parameters set by the user). However, this time we need a proposal distribution with support on the unit interval, since $ \delta \sim \mbox{Beta}(1, 10)$.  Now we define an auxiliary variable $\eta$ which come from the proposal distribution (in the real line) and take the transformation:
 $$\delta = \Phi(\eta),$$
 where $\Phi$ is the standard Gaussian cumulative distribution function. Given that still we cancel out the symmetric part $\phi(\eta^\prime-\eta) =\phi(\eta-\eta^\prime)$ (where $\phi(\cdot)$ is Normal pdf), it follows that the correct acceptance ratio is 
 \begin{equation} 
 	\begin{split}
 		& \mbox{Acceptance Probability}=
