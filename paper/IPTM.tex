...
To be more specific, since ${J}^{(d)}_{\mbox{a}, ij}$ could be either 1 or 0, we divide into two cases as below:
     ...
     \end{equation}
       ...
       \iffalse Or, equivalently, we can apply a random sampling from the Bernoulli probability
              \begin{equation}
              \begin{aligned}
        ...
        Moreover, to further protect from numerical underflow and overflow, we use the numerical approximation of $f(y) = \mbox{log}(1+ e^y)$ as below: 
         ...
         \end{cases}
         \end{equation*}
         where $y = -\Delta T^{(d)}_{i_o^{(d)}J_o^{(d)}}\big(\lambda^{(d)}_{iJ^{(d)}_{\mbox{a}, i[-j]}} - \lambda^{(d)}_{iJ^{(d)}_{\mbox{a}, i[+j]}}\big)- \mbox{log}(\delta\lambda^{(d)}_{ij})$ in this case. \fi
 	...
 	\For{i=1 to I}{
 		...
 		optimize $\alpha$ and $\boldsymbol{m}$ using hyperparameter optimization in \cite{wallach2008structured}
 			...
 			\For{d=1 to D}{
 				...
 				sample the augmented data $(i^{(d)}, J^{(d)}_i)$ using Equation (17) and Equation (18)
 			}
 					\For{n=1 to $N^{(d)}$}{
 						calculate $p^\mathcal{Z}|\mbox{others} =(p_1,...,p_K)$ using Equation (23) or Equation (24)\\
 						draw of $z_n^{(d)}\sim\mbox{multinomial}(p^\mathcal{Z})$}}
 			\For{k=1 to K}{
 				calculate $p^\mathcal{C}|\mbox{others}=(p_1,...,p_C)$ using Equation (26)\\
 				draw $C_k\sim \mbox{multinomial}(p^\mathcal{C})$}
	\For{n=1 to $n_2$}{
 			sample $\mathcal{B}$ using M-H following Section 3.4
 		}
 		\For{n=1 to $n_3$}{
 			sample $\delta$ using M-H following Section 3.4
  		}
 ...
 \end{algorithm}
   ...
   \begin{algorithm}[H]
   	...
   	Input:\\ 1) latent edges $\{\{iJ^{(d)}_i\}_{i \neq i_o^{(d)}}\}_{d=1}^D$ (where $\{i_o^{(d)}\}_{d=1}^D$ from previous backward sample),\\
   	 ...
   	 3) interaction pattern parameters, $\{C_k\}_{k=1}^K$,\\
   	...
   	\For{d=1 to D}{
   		...
   		Only sample the receivers for previously observed sender ($i_o^{(d)}$), which was not inferred.\\
   				\For {j $\neq$ $i_o^{(d)}$} {
   			...
   			draw $\Delta T_{iJ_i} \sim \mbox{Exp}(\lambda_{iJ_i}^{(d)})$, given $\{iJ^{(d)}_i\}_{i=1}^A$\\
   		set $t^{(d)} = t^{(d-1)}+\mbox{min}(\Delta T_{i{J_i}})$, $i^{(d)} = i_{\mbox{min}(\Delta T_{i{J_i}})}$, and $J^{(d)} = J_{i^{(d)}}$ 		
   	}
   \end{algorithm}
   ...
   \begin{itemize}
   	...
   	\begin{minipage}{0.5\textwidth}
   		...
   		\item[-] prior for $\delta\sim \mbox{Gamma}(4, 8)$
...
\end{figure}
\noindent\textcolor{red}{Remaining issues:}\\
Attached GiRplot is the result (with $10^5$ forward and backward samples) using the updated backward sampling, which seems to be much better than before except mean recipients and mean timediff. I have several thoughts on those two, which is again related to the degenerate issue of the model when all latent edges are 0.\\\\
- In the forward sampling (and generative process in general), when all $iJ_i$ is 0, we simply do not move forward (no timestamp to generate) and resample $iJ_i$ until we have $sum(iJ_i) > 0$. By this, we actually generate the data which is ‘denser’ than what we could expect from the small Bernoulli probability ($delta*lambda / (delta*lambda + 1)$ - refer to p.4 of IPTM draft).\\
- Above fact results in slightly large estimates of delta during the inference (shown in the attached ‘deltaplot.pdf’), to fit the denser-than-actual-parameter-network.\\
- Backward sampling accepts the estimated delta as an input and thus generate even ‘denser’ network (making the exponential rate lambda larger), which is probably the reason for smaller values in the GiR variable ‘mean timediff’. 
\\\\
Regarding the mean recipients, there is a clear reasoning behind the failure of GiR.\\
- In the inference, the initial values of $iJ_i$ were sampled the same way as forward sampling according to the generative process.\\
- However, as we update the latent edges, they tend to become 0 (this is the issue we discussed few weeks ago, where the model tends to fit the time part better while pushing the edge part to become outliers). Apparently, the conditional probability of an edge being 1 is much smaller than being 0 (Sec 3.1 in IPTM draft), so as we update $iJ_i$ more and more, the edges are getting close to the expected outcome 0, i.e. the inferred part of $iJ_i$ just goes to zero matrix. \\
- Current GiR updates $iJ_i$ only once (which gave the best results so far), so now backward sampling shows larger mean recipients than forward sampling.  But if I update $iJ_i$ twice, the backward mean recipients drops significantly, which I think will also affect the mean timediff to become much larger.
