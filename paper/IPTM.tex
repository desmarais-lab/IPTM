...
\Delta T_{i{J_i}} \sim \mbox{Exp}(\gamma\lambda_{i{J_i}}^{(d)}(t_+^{(d-1)})).
	\end{equation}
	where $\gamma$ is the parameter reflecting the urgency of the document to be sent with $\gamma \sim \mbox{Beta}(1, |J_i|)$ to adjust for the number of receivers, and $\lambda^{(d)}_{i{J_i}}(t_+^{(d-1)})= \sum\limits_{c=1}^{C} p^{(d)}_c\cdot\mbox{exp}\Big\{\sum\limits_{j \in{J_i}}\boldsymbol{b}^{(c)T}\boldsymbol{x}^{(c)}_{t^{(d-1)}_+}(i, j)\Big\}\cdot \prod\limits_{j \in J_i}1\{j \in \mathcal{A}_{\backslash i}\}$
			...
			draw $\boldsymbol{\theta}^{(d)}\sim \mbox{Dir}(\alpha, \boldsymbol{m})$\\
				\For{n=1 to ${N}^{(d)}$}{
                   ...
                   set $p_c^{(d)} = \frac{\sum\limits_{k: c_k=c} N^{(k|d)}}{N^{(d)}}$
					...
					draw $\Delta T_{iJ_i} \sim \mbox{Exp}(\gamma\lambda_{iJ_i}^{(d)}(t_+^{(d-1)}))$
                    }
...
=\prod_{i\in \mathcal{A}}\prod_{j\in \mathcal{A}_{\backslash i}}\Big(I(i \rightarrow j)\sim \mbox{Ber}(1-\mbox{exp}(-\delta\lambda^{(d)}_{ij}(t_+^{(d-1)})))\Big) \\&\quad\quad\quad\quad\times \prod_{i\in \mathcal{A}_{\backslash J_i=\emptyset}}\Big(\Delta T_{iJ_i}\sim\mbox{Exp}(\gamma\lambda^{(d)}_{iJ_i}(t_+^{(d-1)}))\Big) \times P\Big(\mbox{min}_i (\Delta T_{i{J_i}}) = \Delta T_{i_{o}^{(d)}{J_{o}^{(d)}}}\Big)\\&
=\Big(\prod_{i\in \mathcal{A}}\prod_{j \in \mathcal{A}_{\backslash i }} (1-e^{-\delta\lambda^{(d)}_{ij}(t_+^{(d-1)})})^{I(j \in J_i)}(e^{-\delta\lambda^{(d)}_{ij}(t_+^{(d-1)})})^{1-I(j \in J_i)}\Big)\\&\quad\quad\quad\quad\times \Big(\prod_{i \in\mathcal{A}_{\backslash J_i=\emptyset}}\gamma\lambda^{(d)}_{iJ_i}(t_+^{(d-1)})e^{-(t^{(d)}-t^{(d-1)})\gamma\lambda^{(d)}_{iJ_i}(t_+^{(d-1)})}\Big)\times \Big(\prod_{i\in \mathcal{A}_{\backslash J_i=\emptyset, i_o^{(d)}}}e^{-(t^{(d)}-t^{(d-1)})\gamma\lambda^{(d)}_{i{J_i}}(t_+^{(d-1)})}\Big),
  \end{aligned}
  \end{equation}
  ...
  \begin{equation}
  \begin{aligned}
  &P(\mathcal{I}^{(d)}_{\mbox{a}}, \mathcal{J}^{(d)}_{\mbox{a}}, \mathcal{T}^{(d)}_{\mbox{a}}, i^{(d)}_{\mbox{o}}, J^{(d)}_{\mbox{o}}, t^{(d)}_{\mbox{o}} |\mathcal{I}^{(<d)}_{\mbox{o}}, \mathcal{J}^{(<d)}_{\mbox{o}}, \mathcal{T}^{(<d)}_{\mbox{o}}, \mathcal{Z}, \mathcal{C}, \mathcal{B}, \mu, \delta)\\&=\Big(\prod_{i\in \mathcal{A}}\prod_{j \in \mathcal{A}_{\backslash i }} (e^{\delta\lambda^{(d)}_{ij}(t_+^{(d-1)})}-1)^{I(j \in J_i)}e^{-\delta\lambda^{(d)}_{ij}(t_+^{(d-1)})}\Big)\\&\quad\quad\quad\quad \times \Big(\prod_{i\in \mathcal{A}_{\backslash J_i=\emptyset}} \gamma\lambda^{(d)}_{iJ_i}(t_+^{(d-1)})\Big)\times \Big(e^{-(t^{(d)}-t^{(d-1)})\big(\sum\limits_{i \in \mathcal{A}_{\backslash J_i=\emptyset}}\gamma\lambda^{(d)}_{i{J_i}}(t_+^{(d-1)})+\sum\limits_{i \in \mathcal{A}_{\backslash J_i=\emptyset, i_o^{(d)}}}\gamma\lambda^{(d)}_{i{J_i}}(t_+^{(d-1)})\big)}\Big).
  \end{aligned}
  \end{equation}
For implementation, we need to compute these equations in log space to prevent underflow:
 \begin{equation}
 \begin{aligned}
 &\mbox{log}\Big(P(\mathcal{I}^{(d)}_{\mbox{a}}, \mathcal{J}^{(d)}_{\mbox{a}}, \mathcal{T}^{(d)}_{\mbox{a}}, i^{(d)}_{\mbox{o}}, J^{(d)}_{\mbox{o}}, t^{(d)}_{\mbox{o}} |\mathcal{I}^{(<d)}_{\mbox{o}}, \mathcal{J}^{(<d)}_{\mbox{o}}, \mathcal{T}^{(<d)}_{\mbox{o}}, \mathcal{Z}, \mathcal{C}, \mathcal{B}, \mu, \delta)\Big)\\&=\sum_{i\in \mathcal{A}}\sum_{j \in \mathcal{A}_{\backslash i }} {I(j \in J_i)}\cdot\mbox{log}\Big(e^{\delta\lambda^{(d)}_{ij}(t_+^{(d-1)})}-1\Big) {-\delta\lambda^{(d)}_{ij}(t_+^{(d-1)})}\\&\quad\quad\quad\quad + \sum_{i\in \mathcal{A}_{\backslash J_i=\emptyset}} \mbox{log}(\gamma\lambda^{(d)}_{iJ_i}(t_+^{(d-1)})){-(t^{(d)}-t^{(d-1)})\Big(\sum\limits_{i \in \mathcal{A}_{\backslash J_i=\emptyset}}\gamma\lambda^{(d)}_{i{J_i}}(t_+^{(d-1)})+\sum\limits_{i \in \mathcal{A}_{\backslash J_i=\emptyset, i_o^{(d)}}}\gamma\lambda^{(d)}_{i{J_i}}(t_+^{(d-1)})\Big)}.
 \end{aligned}
 \end{equation}
