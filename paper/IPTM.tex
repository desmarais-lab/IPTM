...
\subsection{Multiplicative Intensity}
...
\begin{itemize}
	...
	\item[2.] Set $t=t^{(d-1)}$, $\boldsymbol{\beta}=\boldsymbol{\beta}^{(c^{(d)})}$ and $\boldsymbol{X}=\{\{\boldsymbol{x}^{*(c^{(d)})}_{t_+}(i, j)\}_{i=1}^{A}\}_{j=1}^{A}$ (i.e. calculate the network statistics at $t^{(d-1)}$, including the timepoint itself)
	\item[3.] (Data augmentation) For each sender $i = 1,...,A$, create a list of receivers $J_i$ by applying the Bernoulli probabilities to every $j \in \mathcal{A}_{\backslash i}$
	\begin{align*} I(i \rightarrow j) \sim \mbox{Ber}\Big(1-\mbox{exp}(-\delta\lambda_{ij}(\boldsymbol{\beta}, \boldsymbol{X}_{ij}))\Big),
	\end{align*}
	where the probability is called a Bernoulli-Poisson (BerPo) link function \citep{zhou2015infinite} and $\delta$ is a tuning parameter to control the number of multicasts.
	\item[4.] For each sender $i = 1,...,A$, generate the time increments $\Delta T_{i{J_i}} \sim \mbox{Exponential}(\lambda_{i{J_i}}(\boldsymbol{\beta}, \boldsymbol{X}_{i{J_i}}))$, where $\lambda_{i{J_i}}(\boldsymbol{\beta}, \boldsymbol{X}_{i{J_i}})= \mbox{exp}\Big\{\sum\limits_{j \in{J_i}}\boldsymbol{\beta}^{T}\boldsymbol{X}_{ij}\Big\}$
	 \item[5.] Set the timestamp $t^{(d)} = t+\mbox{min}(\Delta T_{i{J_i}})$, $i^{(d)} = i_{\mbox{min}(\Delta T_{i{J_i}})}$, and $J^{(d)} = J_{i^{(d)}}$.
\end{itemize}
Then, conditioned upon the existence of a document at some particular time $t^{(d)}$, the probability that the document is sent from $i^{(d)}$ to $J^{(d)} (=J_{i^{(d)}})$ is
\begin{equation}
\begin{split}
\mathcal{L}(\boldsymbol{\beta}^{(c^{(d)})}) &= P\Big(\Delta T_{i^{(d)}{J^{(d)}}}\sim\mbox{Exp}(\lambda_{i^{(d)}{J^{(d)}}}(\boldsymbol{\beta}, \boldsymbol{X}_{i^{(d)}{J^{(d)}}}))\Big)\times P\Big(\mbox{min}(\Delta T_{i{J_i}}) = \Delta T_{i^{(d)}{J^{(d)}}}\Big)\times P\Big(\mbox{Edge creations}\Big)\\&
=\Big(\lambda_{i^{(d)}{J^{(d)}}}(\boldsymbol{\beta}, \boldsymbol{X}_{i^{(d)}{J^{(d)}}})e^{-(t^{(d)}-t^{(d-1)})\lambda_{i^{(d)}{J^{(d)}}}(\boldsymbol{\beta}, \boldsymbol{X}_{i^{(d)}{J^{(d)}}})}\Big)\times \Big(e^{-(t^{(d)}-t^{(d-1)})\sum\limits_{i \neq i^{(d)}}\lambda_{i{J_i}}(\boldsymbol{\beta}, \boldsymbol{X}_{i{J_i}})}\Big)\\ & \quad\quad\times \Big(\prod_{i\in \mathcal{A}}\prod_{j \in \mathcal{A}_{\backslash i }} (1-e^{-\delta\lambda_{ij}(\boldsymbol{\beta}, \boldsymbol{X}_{ij})})^{I(j \in J_i)}(e^{-\delta\lambda_{ij}(\boldsymbol{\beta}, \boldsymbol{X}_{ij})})^{1-I(j \in J_i)}\Big)\\
& = \Big(\lambda_{i^{(d)}{J^{(d)}}}(\boldsymbol{\beta}, \boldsymbol{X}_{i^{(d)}{J^{(d)}}})e^{-(t^{(d)}-t^{(d-1)})\sum\limits_{i \in \mathcal{A}}\lambda_{i{J_i}}(\boldsymbol{\beta}, \boldsymbol{X}_{i{J_i}})}\Big) \times \Big(\prod_{i\in \mathcal{A}}\prod_{j \in \mathcal{A}_{\backslash i }} (e^{\delta\lambda_{ij}(\boldsymbol{\beta}, \boldsymbol{X}_{ij})}-1)^{I(j \in J_i)}e^{-\delta\lambda_{ij}(\boldsymbol{\beta}, \boldsymbol{X}_{ij})}\Big),
\end{split}
\end{equation}
where $\lambda_{i{J}}(\boldsymbol{\beta}, \boldsymbol{X}_{i{J}})=\mbox{exp}\Big\{\sum\limits_{j \in{J_i}}\boldsymbol{\beta}^{(c^{(d)})T}\boldsymbol{x}^{*(c^{(d)})}_{t^{(d-1)}_+}(i, j)\Big\}$ and $\lambda_{ij}(\boldsymbol{\beta}, \boldsymbol{X}_{ij})=\mbox{exp}\Big\{\boldsymbol{\beta}^{(c^{(d)})T}\boldsymbol{x}^{*(c^{(d)})}_{t^{(d-1)}_+}(i, j)\Big\}$. Applying this to the every document in the corpus and taking the log, then we obtain the joint probability, the full likelihood function as:
\begin{equation}
\begin{split}
