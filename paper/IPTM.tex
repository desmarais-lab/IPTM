...
&=\prod_{d=1}^{D} \Big(\lambda^{(d)}_{i^{(d)}{J^{(d)}}}e^{-\Delta T^{(d)}_{i_o^{(d)}J_o^{(d)}}\sum\limits_{i \in \mathcal{A}}\lambda^{(d)}_{i{J_i}}}\Big) \times \Big(\prod_{i\in \mathcal{A}}\prod_{j \in \mathcal{A}_{\backslash i }} (e^{\delta\lambda^{(d)}_{ij}}-1)^{I(j \in J_i)}e^{-\delta\lambda^{(d)}_{ij}}\Big),
\end{aligned}
\end{equation}
