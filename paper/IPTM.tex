	...
	\end{algorithm}
	  \begin{figure}[ht]
	  	...
	  	\scalebox{0.65}{  \begin{tikzpicture}
	  		...
	  		\node[main] (x) [right=of theta,label=center:$\boldsymbol{x}^{(c)}_{t_+^{(d-1)}}$] { };
	  		\node[main] (lambda) [below=of x,label=center:$\boldsymbol{\lambda}^{(d)}(t_+^{(d-1)})$] { };
	  		\node[main] (mu) [above=of x,label=center:$\mu$] { };
	  		\node[main] (c) [right=of mu,label=center:$c_k$] { };
	  		\node[main] (e) [right=of lambda,label=center:${(i, J_i)}^{(d)}$] { };
	  		\node[main] (delta) [right=of e,label=center:$\delta$] { };
	  		\node[main] (b) [above=of delta,label=center:$\boldsymbol{b}^{(c)}$] { };
	  		...
	  		(z) edge [connect] (x)
	  		(z) edge [connect] (lambda)
	  		(mu) edge [connect] (x)
	  		(x) edge [connect] (lambda)
	  		(c) edge [connect] (x)
	  		(c) edge [connect] (lambda)
