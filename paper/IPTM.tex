 ...
 \begin{algorithm}[H]
	...
	\For{o=1 to O}{
	 ...
	 obtain global latent variables ($\mathcal{B}$, $\mathcal{C}$, $\delta$)  and local variables  ($\mathcal{Z}^{1:(D-1)}$, $\mathcal{J}_a^{1:(D-1)}$)\\
		...
		\For{i = 1 to A} {
		\For{j $\neq$ i}{
			set $P(\mathcal{J}_{a, ij}^{(D)} = 1|\mathcal{J}_{a, ij}^{(1:(D-1))}) = \frac{\sum_{d=1}^{D-1} I(\mathcal{J}^{(d)}_{a, ij}=1)}{D-1}$\\
			$\mathcal{J}_{a,ij}^{(D)} \sim \mbox{Bernoulli}(P(\mathcal{J}_{a, ij}^{(D)} = 1|\mathcal{J}_{a, ij}^{(1:(D-1))}))$
		}
		}
		...
		- set $t_o^{(D)} = t_o^{(D-1)} + \Delta T$ \\
							 \For{$n$ = 1 to $\bar{N}^{(D)}$}{
							 	Sample $z_n^{(D)}$ using Equation (27): $$P(z^{(D)}_n=k|\mathcal{Z}_{\backslash D, n},   \mathcal{C},   \mathcal{B}, \mathcal{W}^*, \mathcal{J}_{\mbox{a}}, \mathcal{I}_{\mbox{o}}^{(D)}, \mathcal{J}_{\mbox{o}}^{(D)}, \mathcal{T}_{\mbox{o}}^{(D)}, \mathcal{Z}_{1:(D-1)}, \boldsymbol{w}_{1:(D-1)}, \beta, \boldsymbol{u}, \alpha, \boldsymbol{m}).$$ 
							 }
							 Recalculate $p_c^{D}$ from the updated $z^{(D)}$
							 \For{$i  \neq i^{*(d)}_o$}{
							 	\For{$j\neq i$ }{
							 		Sample $\mathcal{J}_{ij}^{(d)}$ using Equations (19) and (20):
							 		$$  P(\mathcal{J}^{(D)}_{ij} = {J}^{(D)}_{ij}|\mathcal{J}^{(D)}_{i\backslash j}, \mathcal{Z}, \mathcal{C},   \mathcal{B}, \delta, \mathcal{W}^*,  \mathcal{J}^{(D)}_{\mbox{a}, -i}, \mathcal{I}_{\mbox{o}}^{(D)}, \mathcal{J}_{\mbox{o}}^{(D)}, \mathcal{T}_{\mbox{o}}^{(D)}). $$ 
