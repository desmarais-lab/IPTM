...
Finally for implementation, we need to compute these equations in log space to prevent underflow:
 ...
 To actually perform inference, we therefore want to use Gibbs sampling, where we will sequentially resample the values of each of our latent variables from their posterior distribution, conditional on all of our other variables.
    \subsection{Resampling the augmented data $\mathcal{J}_{\mbox{a}}$} \label{subsec: Data augmentation}
      ...
      \end{aligned}
      \end{equation}
      where we replace typical use of $(-d)$ to $(<d)$ on the right hand side of the conditional probability, due to the fact that $d^{(th)}$ document only depends on the past documents, not on the future ones (see the note below Equation (11)).\\ \newline
To be more specific, since ${J}^{(d)}_{\mbox{a}, ij}$ could be either 1 or 0, we divide into two cases as below:
     ...
     \end{aligned}
     \end{equation}
       where $\lambda^{(d)}_{iJ^{(d)}_{\mbox{a}, i[-j]}}$ meaning similarly that $\lambda^{(d)}_{iJ^{(d)}_{\mbox{a}, i}}$ is calculated with $j^{th}$ element of $J_{\mbox{a}, i}^{(d)}$ fixed as 0. \\\\ \newline Now we can use multinomial sampling using the two probabilities, Equation (17) and Equation (18). Again, we would calculate the probabilities in the log space to prevent from numerical underflow. Or, equivalently, we can apply a random sampling from the Bernoulli probability
              \begin{equation}
              \begin{aligned}
 	...
 	\For{i=1 to I}{
 		...
 		optimize $\alpha$ and $\boldsymbol{m}$ using hyperparameter optimization in \cite{wallach2008structured}
 			...
 			\For{d=1 to D}{
 				...
 				sample the augmented data $(i^{(d)}, J^{(d)}_i)$ 
 			}
 			}
 				\For{d=1 to D}{
 					\For{n=1 to $N^{(d)}$}{
 ...
 \end{algorithm}
   ...
   \begin{itemize}
   	...
   	\begin{minipage}{0.5\textwidth}
   		...
   		\item[-] $\sigma_Q^2$ (proposal variance for $\delta$) = 2   		
   	\end{minipage}
   \end{itemize}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=1\textwidth]{GiRplot.pdf} 
	\label{fig:GiR}
