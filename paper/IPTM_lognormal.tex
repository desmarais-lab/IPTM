...
Our inference goal is to draw samples from the posterior distribution
 ...
 \begin{algorithm}[H]
	...
	\For{n=1 to $n_2$}{
 			sample $\mathcal{B}$ and $\delta$ using Metropolis-Hastings following Section \ref{subsec: Resampling B}	}
  		\For{n=1 to $n_3$}{
  				sample $\boldsymbol{\eta}$ and $\sigma_T^2$ using Metropolis-Hastings following Section \ref{subsec: Resampling eta}
  				}
		}
...
Z^{l}(\delta,\mbox{log}(\lambda_{ij}^{(d)})) &= \frac{1}{\prod_{j \in \mathcal{A}_{\backslash i}}   \frac{ \exp\{-(\delta+\mbox{log}(\lambda_{ij}^{(d)}))\}}{\exp\{-(\delta+\mbox{log}(\lambda_{ij}^{(d)}))\} + 1}}.
 ...
 $P(\mathcal{J}^{(d)}_{\mbox{a}}, i^{(d)}_{\mbox{o}}, J^{(d)}_{\mbox{o}}, t^{(d)}_{\mbox{o}}| z^{(d)}_n=k, \mathcal{Z}_{\backslash d, n}, \mathcal{C}, \mathcal{B}, \delta, \boldsymbol{\eta}, \sigma_T^2)$ is Equation (\ref{eqn:jointIJT})
   ...
   \subsubsection{Resampling $\mathcal{B}$ and $\delta$}  \label{subsec: Resampling B}
  ...
  \subsubsection{Resampling $\boldsymbol{\eta}$ and $\sigma_T^2$}  \label{subsec: Resampling eta}
        	...
        	5) time-increment parameters ($\boldsymbol{\eta}$, $\sigma_T^2$).\\\\
        		...
        		\For{n=1 to $ {N}^{(d)}$}{
        			...
        			\For{i=1 to $A$}{
        				...
        				draw $J_i^{(d)} \sim \mbox{Gibbs measure}(\{\lambda_{ij}^{(d)}\}_{j=1}^A,\delta)$\\
        				calculate $\boldsymbol{y}_{iJ_i}^{(d)}$ (including $\boldsymbol{x}_{iJ_i}^{(d)}$)\\
        				draw $\Delta T^{(d)} _{iJ_i} \sim \mbox{Lognormal}(\boldsymbol{\eta}^T\boldsymbol{y}_{iJ_i}^{(d)}, \sigma_T^2)$
        			}
        		  set $i_o^{(d)} = i_{\mbox{min}(\Delta T^{(d)} _{i{J_i}})}$, $J_o^{(d)} = J_{i^{(d)}}$, and
        		  $t_o^{(d)} = t_o^{(d-1)}+\mbox{min}(\Delta T^{(d)} _{i{J_i}})$
        			 	}
        \end{algorithm}
