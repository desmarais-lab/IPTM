...
We evaluated the IPTM's ability to predict ties in textual communications from either the Dare County email network or the Enron data set, conditioned on the text of those emails and ``training" part of the data. We separately formed a test split of each three components---author, recipients, and timestamps---by randomly selecting emails with probability 0.2 such that an email may observe all, some, or none of tie information. Any missing variables were imputed by drawing samples from their conditional posterior distribution using an appropriately-modified version of the Metropolis-within-Gibbs algorithm in Section \ref{sec:Inference}. We then ran inference to update the latent variables given the imputed data and iterated the two---imputation and inference---multiple times to obtain enough number of new data. Algorithm \ref{alg:PPE} outlines this procedure.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		number of new data to generate $R$,\\
			...
			\vskip 0.1in
		\textbf{Test splits:}	
		\STATE Draw ``test1" emails with $p=0.1$ and hide $a_{d:d \in {\mbox{test1}}}$
		\STATE Draw ``test2" emails with $p=0.1$ and hide $\boldsymbol{r}_{d:d \in {\mbox{test2}}}$
		\STATE Draw ``test3" emails with $p=0.1$ and hide $t_{d:d \in {\mbox{test3}}}$
		\vskip 0.1in
	...
	\IF{$d \in \{\mbox{(test1)} \cap\mbox{(test3)}\}$---\textcolor{red}{CASE 1}}
	\FOR{$a=1$ {\bfseries to} $A$}
	\STATE Draw $\log(\tau_{ad}) \sim N(\mu_{ad}, \sigma^2_\tau)$
	\ENDFOR
	\STATE Set author and timestamp following Section \ref{subsubsec:Actual Data} \\
     $(a_d, t_d)$ = $(\mbox{argmin}_a(\tau_{ad}), t_{d-1}+\tau_{a_dd})$\\
	\textcolor{red}{Directly from generative process}
	\ENDIF
		\IF{$d \in \{\mbox{(test1)} \cap\mbox{(test3)}^c\}$---\textcolor{red}{CASE 2}}
		\FOR{$a=1$ {\bfseries to} $A$}
		\STATE Using Equation (17), compute $P(a_d = a)=$ \\$ \varphi_{\tau}(\tau_{ad}; \mu_{ad}, \sigma_\tau^2) \prod
	\limits_{a'\neq a}\big(1-\Phi_{\tau}(\tau_{a d}; \mu_{a' d}, \sigma_\tau^2) \big)$
		\ENDFOR
		\STATE Draw $a_d$ from the probability $\{P(a_d = a)\}_{a=1}^A$
			\textcolor{red}{Given the observed time, who is most likely to be the author?}
		\ENDIF
 	\IF{$d \in \{\mbox{(test1)}^c \cap\mbox{(test3)}\}$---\textcolor{red}{CASE 3}}
			\STATE Draw $\log(\tau_{a_dd}) \sim N(\mu_{a_dd}, \sigma^2_\tau)$\\
				\textcolor{red}{Given the observed author, what is the associatied time-increment?}
			\ENDIF
 	\IF{$d \in \{\mbox{(test2)}\}$}
 	\STATE Draw $\boldsymbol{r}_d \sim \mbox{Gibbs}( \delta, \boldsymbol{\lambda}_{a_dd})$\\
 	\textcolor{red}{can't directly sample from Gibbs if $A$ large. Alternatively,}
 	\STATE Set $\boldsymbol{r}_d = \boldsymbol{u}_{a_dd}$
 	\ENDIF
			
	\vskip 0.1in		
	\STATE Run one outer iteration of inference algorithm following Section \ref{sec:Inference} and update the parameters $(\boldsymbol{l}, \boldsymbol{z}, \boldsymbol{b}, \boldsymbol{\eta},\delta, \boldsymbol{u})$ given the imputed data
		\ENDFOR
		\STATE Store every $r^{th}$ new data
				\ENDFOR
		\end{algorithmic}
\end{algorithm}
