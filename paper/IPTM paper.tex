...
\begin{itemize}
	...
	\item[3.] Generate $\Delta T \sim \lambda(\boldsymbol{\beta}, \boldsymbol{X})$, where $\lambda(\boldsymbol{\beta}, \boldsymbol{X})=\sum\limits_{i=1}^{A}\sum\limits_{j=1}^{A} \mbox{exp}\Big\{\boldsymbol{\beta}^{T}\boldsymbol{X}_{ij}\Big\}$
	 ...
	 \end{align*}
where $\lambda_i(\boldsymbol{\beta}, \boldsymbol{X}_i)=\sum\limits_{j \in \mathcal{A}_{\backslash i}} \mbox{exp}\Big\{\boldsymbol{\beta}^{T}\boldsymbol{X}_{ij}\Big\}$
...
\end{comment}
\item[7.] if $|J^{(d)}|>1$, select the receiver set  $J^{(d)} \subseteq \mathcal{A}_{\backslash i}$ randomly using probabilities
...
\item[$7^{\prime}$.] (Using approximation in (7)) if $|J^{(d)}|>1$, select the receiver set  $J^{(d)} \subseteq \mathcal{A}_{\backslash i}$ randomly using probabilities
...
\end{algorithm}
 ...
 \newline We use the statistics defined in \cite{PerryWolfe2012} as the components of $\boldsymbol{x}^{*(c)}_t(i, j)$ (refer to Fig.3 of \cite{PerryWolfe2012} attached), then additionaly use the intercept (to estimate the baseline intensities), outdegree, and indegree (to measure the popularity and centrality). For simplicity, we add the four triadic statistics (2-send, 2-receive, sibling, cosibling) and define it as `triangle' statistic. Moreover, to maintain the consistent scale of the statistics, we divide by the total number of documents corresponding to the condition $c^{(d)}=c$ and $t^{(d)}<t$, which we refer to $D^{ct}$.\\
 \begin{itemize}
 	...
 	\item [4.] $\mbox{triangle}_t(i, j)=\sum\limits_{d: c^{(d)}=c}\sum\limits_{h \neq i, j}\Big(\sum\limits_{t^{(d)}<t}  I\{i\rightarrow h \mbox{ or } h\rightarrow j \}\Big)\Big(\sum\limits_{t^{(d)}<t} I\{j\rightarrow h \mbox{ or } h\rightarrow j\}\Big)/(D^{ct})^2$
