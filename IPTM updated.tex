...
\end{equation} and is concave in $\alpha \textbf{m}$, we will use the estimation technique called ``new fixed-point iterations using the Digamma recurrence relation'' in \cite{wallach2008structured}, based on Minka’s fixed-point iteration \citep{minka2000estimating}.
\subsection{New fixed-point iterations using the Digamma recurrence relation}
In this section, we summarize Chapter 2 of \cite{wallach2008structured} to illustrate the basic steps and equations used for our optimization. \\
...
The optimization procedure in Section 5.1 was applied to Vance county email data, using the same settings as before: $K=20$ topics, $\alpha=5$ and $C=3$. However, now we assume asymmetric base prior $\mathbf{m}$.
\clearpage
\section*{APPENDIX}
