...
In this section, we summarize Chapter 2 of \cite{wallach2008structured} to illustrate the basic steps and equations used for our optimization. Basically, we want to find the optimal hyperparameter $[\alpha\boldsymbol{m}]^*$ given the data $\mathcal{D}$ such that the probability of the
data given the hyperparameters $P(\mathcal{D}|\alpha\boldsymbol{m})$ is maximized at $[\alpha\boldsymbol{m}]^*$. The evidence is given by 
\begin{equation}
P(\mathcal{D}|\alpha\boldsymbol{m})=\prod_{d=1}^{D}\frac{\Gamma(\alpha)}{\Gamma(N_{\cdot|d}+\alpha)}\prod_{k=1}^{K}\frac{\Gamma(N_{k|d}+\alpha m_k)}{\Gamma(\alpha m_k)}
...
the average VI distance between all 10 unique pairs of interaction pattern and topic assignments from the 5 Gibbs runs for the symmetric and asymmetric model, respectively. We also calculated the between-model
...
\begin{table}[ht]
	...
	\begin{tabular}{|c|c|c|} 
		...
		\hline
		Symmetric \textbf{$\boldsymbol{m}$} &  $5.350\pm 0.053$&  $5.344 \pm 0.084$\\
		\hline
		Asymmetric \textbf{$\boldsymbol{m}$} & - & $5.308 \pm 0.079$\\
		\hline
	\end{tabular}
	\caption {Average VI distances between 5 runs of symmetric and asymmetric model with $K=20$}
	\label{table:VIforZ}
\end{table}
\normalsize
\footnotesize
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.75\textwidth]{topics.pdf} 
		\includegraphics[width=0.75\textwidth]{topic.pdf} 
	\caption{Posterior distribution of  $\mathcal{Z}$ for Vance county emails}
	\label{fig:Vancebarplot}
\end{figure}
\footnotesize
\begin{table}[ht]
	...
	\scalebox{0.8}{	\begin{tabular}{|l||l||l||l||l|}
			\hline
			{\textbf{Topic 5} (0.1213)}&{\textbf{Topic 6} (0.1136)}&{\textbf{Topic 13} (0.0598)}&{\textbf{Topic 2} (0.0571)}&{\textbf{Topic 8} (0.0531)}\\
			\hline\hline
		operations &  office & directory & electronic &  message\\
			emergency &  heads &  required &  review &fax \\
			time & manager & phones & street & communications \\
			response & pursuant & copies &atcom &  chapter\\
			ncgs &attachments & dial &  department &  center \\
				request &tomorrow &  expect &  additional & directory \\
			morning & received &tuesday & years & folks\\
			finance & sheet &  dss &  forward & audit \\
				latest & internal &  october & compliance & cecd \\ 
				financial &henderson&   held&  signups &  performance\\
			\hline
		\end{tabular}}
...
\begin{table}[ht]
	...
	\scalebox{0.8}{	\begin{tabular}{|l||l||l||l||l|}
			\hline
		{\textbf{Topic 5} (0.1134)}&{\textbf{Topic 15} (0.0844)}&{\textbf{Topic 2} (0.0811)}&{\textbf{Topic 6} (0.0771)}&{\textbf{Topic 18} (0.0666)}\\
			\hline\hline
			operations & request & message & cutover &henderson \\
			meeting & good & emergency & services & office \\
			records & pursuant & electronic & advised & church\\
			ncgs & director & review & directory & public\\
			subject & center & time & phase &best\\
			morning & additional & manager & finishing &department\\
			heads & email & tomorrow & october &level\\
			finance & advised & young & training &created\\
			latest & wireless & commissioners & rest &technology\\
			financial & heads & compliance & finalized &sheet\\
				\hline
		\end{tabular}}
